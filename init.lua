...
local function ptime(name)
	...
	if player:get_attribute("ptime") == nil or player:get_attribute("ptime") == "" then
...
minetest.register_on_joinplayer(function(player)
	...
	if player:get_attribute("ptime") == "day" then
        player:override_day_night_ratio(1)
	    minetest.chat_send_player(player_name, "-!- Perma Day is enabled")
	elseif player:get_attribute("ptime") == "night" then
	    player:override_day_night_ratio(".1")
	    minetest.chat_send_player(player_name, "-!- Perma Night is enabled")
    end
end)
        ...
        action = function(player)
            ...
            if minetest.check_player_privs(player_name, {daylight=true}) then
                ...
                elseif player:get_attribute("ptime") == "day" then
                    player:set_attribute("ptime", "night")
                    player:override_day_night_ratio(.1)
                    minetest.chat_send_player(player_name, ("-!- Perma Night enabled"))
                elseif player:get_attribute("ptime") == "night" then
                    player:set_attribute("ptime", "")
                    player:override_day_night_ratio(nil)
                    minetest.chat_send_player(player_name, ("-!- Perma Time disabled"))
                end
            else
